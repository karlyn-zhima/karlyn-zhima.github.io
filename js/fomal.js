...
function createtime1() {
  var grt = new Date("03/18/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 芝麻") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('芝麻的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (m == 9 && dd == 3) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝芝麻" + (y - 2001).toString() + "岁生日快乐！🥝");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 9 && dd == 15) {//小猫咪生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝諒之" + (y - 2004).toString() + "岁生日快乐！🐱");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  var start = new Date("03/18/2025 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("03/18/2025 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' title='我与我周旋久'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' title='时光时光慢些吧'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    ...
    --mobileday-bg: url(https://sourcebucket.s3.bitiful.net/img/home_bg.webp);
